...
class KYCpage extends React.Component {
  ...
  onClickForm = async (e) => {
    ...
    if (email2 !== '' && name !== '' && message !== '') {
      ...
      try {
        await axios.post('https://tokedo.io/contactForm', {
          email2: email2,
          message: `${name}\n\n${message}`,
          department: 'tokedo'
        })
  ...
  render () {
    const { t } = this.props
    const { email2, name, message, status } = this.state
    return (
            ...
            <form className='kycForm bottom d-flex flex-row flex-h-between' onSubmit={this.onClickForm}>
              ...
              <div className='input-box pure-u-1 d-flex flex-column flex-v-center'>
                ...
                <div className='marginTop pure-u-1 pure-u-sm-1 pure-u-md-1-2 pure-u-lg-1-2 pure-u-xl-1-3'>
                  ...
                  <input className=' text shadow pure-u-1' type='text' name='email2' value={email2} onChange={this.onChangeText} />
                </div>

                <div className='marginTop pure-u-1 pure-u-sm-1 pure-u-md-1-2 pure-u-lg-1-2 pure-u-xl-1-3'>
                  <label>{t('Name')}</label><br />
                  <input className=' text shadow pure-u-1' type='text' name='name' value={name} onChange={this.onChangeText} />
                </div>
                <div className='marginTop pure-u-1 pure-u-sm-1 pure-u-md-1-2 pure-u-lg-1-2 pure-u-xl-1-3'>
                  <label>{t('Message')}</label><br />
                  <input className=' text shadow pure-u-1' type='text' name='message' value={message} onChange={this.onChangeText} />
                </div>
                <div className='marginTop pure-u-1 pure-u-sm-1 pure-u-md-1-2 pure-u-lg-1-2 pure-u-xl-1-3'>
