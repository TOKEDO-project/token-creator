...
class HelpPage extends React.Component {
  ...
  componentDidMount () {
    ...
    if (typeof hash === 'string' && hash.length > 0) {
      ...
      window.scrollTo(0, targetForScroll.offsetTop - 15)
    }
  }
    ...
    return (
      <div id='helpPage' className='pure-u-1 d-flex flex-column flex-v-center pages'>
        <div className='maxWidth box shadow pure-u-1 pure-u-sm-1 pure-u-md-1 pure-u-lg-22-24 pure-xl-15-24'>
          ...
          <p>{t('The application is open source and be found at ')}<a href='https://github.com/TOKEDO-project/token-creator' target='_blank'>GitHub.org</a>.</p>

          <h3>{t('Token Design')}</h3>
          <p><span>{t('How to choose the Token Name?')}</span></p>
          <p>{t('This is the first step to creating your token. Remember:')}</p>
          <ul>
            <li>{t('The name can start with a capital letter ')}</li>
            <li>{t('It should not have any blank spaces, so use just one word, or use an underscore instead of space.')}</li>
            <li>{t('The name of the token must be unique, as it will the name everyone will use to address your token. Think about coins like Ethereum or Bitcoin. Choose a name that represents your project.')}</li>
          </ul>
          <p><span>{t('How to choose the Token Ticker?')}</span></p>
          <p>{t('You need to choose a symbol, or ticker, for your token. For example, Ethereum is identified with the ETH ticker, Bitcoin with BTC, Euro with EUR, Dollar with USD. Find a ticker that has a connection to your name. The ticker must be all capitalized and can have 3 or 4 characters only. Choose wisely to avoid similarities that could cause confusion. Check on ')}<a href='https://coinmarketcap.com/' target='_blank'>Coinmarketcap</a>{t(' or other websites if a ticker is already taken and pick one that is not in use.')}</p>

          <p><span>{t('What are Token Decimals?')}</span></p>
          <p>{t('Token decimals determine the number of digits that follow the decimal separator of your token. You can specify any number from zero to 18. The maximum number of decimals that you can set is 18, because the Token Creator will generate a token on the Ethereum blockchain and on the Ethereum network the maximum amount of decimals that a token can have is 18.')}</p>
          ...
          <p>{t('At the end of the process you will see a message informing you that the token is now live and you will be given the address of your Token’s contract. You can copy the address and check it on Etherscan and similar sites. You can now proceed to create the contract of the token sale.')}</p>
          ...
          <p>{t('This passage is required to add the different token sale contracts for your token. You need to determine the total amount of tokens you want to allocate for sale. This is a preliminary passage to initialize the token sales that you will then create and for which you will be able to choose different parameters such as price, token amount, start/end date, etc.')}</p>

          <p>{t('To create the Project you will need to send three transactions: ')}</p>
          <ul>
            <li>{t('The first transaction will deploy to the blockchain the Project’s smart contract. ')}</li>
            <li>{t('The second transaction will send the amount of tokens that you want to sell. You can now choose the total amount of tokens you want to allocate to the sale. For example you can send 50% of the total supply. In the future you can change this quantity. You will have the ability to add more tokens or to remove some of them. Of course this amount can never exceed the total supply.')}</li>
          ...
          <p>{t('Now that you have created a Project and allocated some tokens for sale, you can create different token sale contracts. Click on ')}<span>{t('Add token sale')}</span>{t(' on your dashboard. You are now required to set a ')}<span>{t('Price')}</span>{t(' for your token. Remember: you can have different token sales that differ from one to another in different aspects, one of them being the price of the token. This way, you can create different contracts to sell your tokens at different prices (special discounts, reservation contracts, private sales, presales, etc). For the time being you are required to set the price in Ether.')}</p>
          ...
          <p id='unlockSection'><span>{t('How to unlock the Token? What happens when it’s unlocked?')}</span></p>
